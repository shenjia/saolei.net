...
define('FROM_PATH', '/Users/zhangshenjia/Desktop');
...
if ($fileh) {
    ...
    while ($line = fgets($fileh)) {
        ...
        if (videoExists($video)) continue;
        if (!checkUser($video['Video_Player'])) {
        ...
        if (file_exists($filepath)) {
            ...
            if (is_int($info)) {
                if (!in_array($video['Video_Player'], $problem_authors)) {
                    error($video['Video_Id'] . " parse failed. author [ " . $video['Video_Player'] . " ] " . $filepath);
                }
                continue;
            }
            if (videoDuplicated($info)) {
                error($video['Video_Id'] . ' : already uploaded!! hash [' . $info['hash'] . ']');
                continue;
            }
            /* 放过已上传的小3bv录像
            ...
            */
            if (!insertUserSig($video, $info) && !in_array($info['player'], $skip_signs)) {
                error($video['Video_Id'] . ' : invalid signature!' . $info['player']);
                continue;
            }
            //echo date('Y-m-d',$info['create_time']).' / '.$video['Video_Time'].PHP_EOL;
            //echo $video['Video_Playero_Id'] . " ok!" . PHP_EOL;
            //var_dump($video,$info);die();
            insertVideo($video, $info);
            insertVideoInfo($video, $info);
            insertVideoStat($video, $info);
            insertVideoScore($video, $info);
            increaseUserVideoStat($video, $info);
            if ($info['noflag']) insertVideoScore($video, $info, true);
            echo '.';
            continue;
        } else {
            error($video['Video_Id'] . " file missed. " . $filepath);
            continue;
        }
        //insertVideoSig($video);
        //echo $video['Player_Id'] . ' : ' . $video['Player_Name_Chinese'] . PHP_EOL;
    }
    unset($line);
    fclose($fileh);
}
...
function insertVideo(&$video, &$info)
{
    $sql = sprintf("insert video (id, level, user, hash, status, review_user, review_time, create_time) 
        ...
        $review_time = $video['Video_Check'] == 'True' ? timestamp($video['Video_CheckTime']) : NULL,
        $create_time = timestamp($video['Video_Time']) 
    );
    $ret = mysql_query($sql);
    if (!$ret) error('insert video error!' . $sql);
}
...
function insertVideoInfo(&$video, &$info)
{
    $sql = sprintf("insert video_info (id, filepath, signature, software, version, noflag, board, board_3bv, real_time, create_time) 
        ...
        $id = (int) $video['Video_Id'],
        $filepath = copyVideoFile($video, $info),
    ...
    $ret = mysql_query($sql);
    if (!$ret) error('insert video info error!' . $sql);
}
...
function insertVideoScore(&$video, &$info, $nf = false)
{
    ...
    $ret = mysql_query($sql);
    if (!$ret) error('insert video score error!' . $sql);
}
...
function insertVideoStat(&$video, &$info)
{
    ...
    $ret = mysql_query($sql);
    if (!$ret) error('insert video stat error!' . $sql);
}
...
function insertUserSig(&$video, &$info) 
{
    ...
    if ($sig != null) {
        ...
        if ($ret) {
            ...
            error($video['Video_Id'] . ' : insert user_sig error!');
            return false;
        }
...
function increaseUserVideoStat(&$video, &$info) 
{
    ...
    $ret = mysql_query($sql);
    if (!$ret) error('increase use video stat error!' . $sql);
}
...
function videoExists($video)
{
    ...
    $ret = mysql_query($sql);
    return mysql_fetch_assoc($ret)['count'] == '1';
}
...
{
    $time = timestamp($video['Video_Time']);
...
function checkUser($id)
{
    ...
    $res = mysql_query($sql);
    return mysql_fetch_assoc($res)['count'] == '1';
}

function timestamp($string) 
{
    return strtotime(explode('.', $string)[0]);   
}
